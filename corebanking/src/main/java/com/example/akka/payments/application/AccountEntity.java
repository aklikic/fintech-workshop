...
import com.example.akka.payments.domain.AccountEvent;
import com.example.akka.payments.domain.AccountState;
...
public class AccountEntity extends EventSourcedEntity<AccountState, AccountEvent> {
    ...
    public Effect<ApiAccount> getAccount() {
        if (currentState().isEmpty()) {
    ...
    public Effect<AuthorisationResponse> authoriseTransaction(AuthorisationRequest request) {
        if (currentState().isEmpty()) {
            ...
            } else if (!currentState().isAvailableBalance(request.amount())) {
    ...
    public Effect<Done> captureTransaction(String transactionId) {
        if (currentState().isEmpty()) {
            return effects().error("Account not found");
        } else if(!currentState().getAuthorisation(transactionId).isPresent()) {
            //deduplication
    ...
    public record AuthorisationResponse(Optional<String> authCode, AuthorisationResult authResult, AuthorisationStatus authStatus){
